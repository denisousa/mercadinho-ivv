import React from 'react';
import {
  Container,
  Typography,
  Grid,
  Card,
  CardContent,
  CardMedia,
  Button,
  Box,
} from '@mui/material';
import { useNavigate } from 'react-router-dom';

// Mock featured products
const featuredProducts = [
  {
    id: 1,
    name: 'Arroz Integral',
    description: 'Arroz integral de alta qualidade',
    price: 50,
    image: '/images/arroz.jpg',
    category: 'Alimentos',
  },
  {
    id: 2,
    name: 'Feijão Carioca',
    description: 'Feijão carioca tipo 1',
    price: 40,
    image: './images/feijao.png',
    category: 'Alimentos',
  },
  {
    id: 3,
    name: 'Soja	',
    description: 'Soja 500ml',
    price: 30,
    image: './images/soja.png',
    category: 'Alimentos',
  },
  {
    id: 4,
    name: 'Detergente',
    description: 'Detergente líquido 500ml',
    price: 30,
    image: './images/detergente.png',
    category: 'Limpeza',
  },
  {
    id: 4,
    name: 'Escova de dente',
    description: 'Escova de dente 100ml',
    price: 30,
    image: './images/escova-de-dente.png',
    category: 'Higiene',
  },
  {
    id: 5,
    name: 'Leite',
    description: 'Leite 1L',
    price: 30,
    image: './images/leite.jpeg',
    category: 'Bebidas',
  },

];

const Home = () => {
  const navigate = useNavigate();

  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <br />
      <Box sx={{ textAlign: 'center', mb: 6 }}>
        <Typography variant="h3" component="h1" gutterBottom>
          Bem-vindo ao Mercadinho do IVV
        </Typography>
        <Typography variant="h5" color="text.secondary" paragraph>
          Troque seus pontos por produtos de qualidade
        </Typography>
      </Box>
      <br />

      <Typography variant="h4" component="h2" gutterBottom sx={{ mb: 4 }}>
        Produtos em Destaque
      </Typography>

      <Grid container spacing={4}>
        {featuredProducts.map((product) => (
          <Grid item xs={12} sm={6} md={4} key={product.id}>
            <Card>
              <CardMedia
                component="img"
                height="200"
                image={product.image}
                alt={product.name}
                sx={{
                  height: 200,
                  objectFit: 'contain',
                  backgroundColor: '#f5f5f5',
                  padding: '10px'
                }}
              />
              <CardContent>
                <Typography gutterBottom variant="h5" component="div">
                  {product.name}
                </Typography>
                <Typography variant="body2" color="text.secondary" paragraph>
                  {product.description}
                </Typography>
                <Typography variant="h6" color="primary">
                  {product.price} pontos
                </Typography>
                <Button
                  variant="contained"
                  color="secondary"
                  fullWidth
                  sx={{ mt: 2 }}
                  onClick={() => navigate('/products')}
                >
                  Comprar Agora
                </Button>
              </CardContent>
            </Card>
          </Grid>
        ))}
      </Grid>

      <Box sx={{ mt: 6, p: 4, bgcolor: 'primary.main', color: 'white', borderRadius: 2 }}>
        <Typography variant="h4" gutterBottom>
          Como Funciona?
        </Typography>
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <Typography variant="h6">1. Acumule Pontos</Typography>
            <Typography>
              Ganhe pontos em suas compras e atividades diárias
            </Typography>
          </Grid>
          <Grid item xs={12} md={12}>
            <Typography variant="h6">2. Escolha Produtos</Typography>
            <Typography>
              Navegue por nossa seleção de produtos de qualidade
            </Typography>
          </Grid>
          <Grid item xs={12} md={12}>
            <Typography variant="h6">3. Troque por Produtos</Typography>
            <Typography>
              Use seus pontos para adquirir os produtos desejados
            </Typography>
          </Grid>
        </Grid>
      </Box>
    </Container>
  );
};

export default Home; 